...
import { AYNA_CLERK_ORG_ID, getOrCreateCurrentUser, requireAdmin } from "@/lib/auth";
...
export async function inviteAynaMember(emailAddress: string, role: "org:member" | "org:admin" = "org:member") {
  ...
  const email = z.string().email().parse(emailAddress.trim());
  ...
  const invitation = await client.organizations.createOrganizationInvitation({
    ...
    emailAddress: email,
    role,
    redirectUrl: `${(process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000").replace(/\/$/, "")}/`,
  });
